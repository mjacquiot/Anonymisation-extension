...
(function() {
  ...
  // Détecte si un mot est suivi par un autre mot capitalisé ou un connecteur français et un mot capitalisé
  ...
  function pseudonymizeText(text, sessionState, config = { forcedElements: [], excludedElements: [] }) {
    ...
    // Elle cherche un caractère majuscule français précédé de début de ligne ou de non-lettre,
    // suivi de lettres minuscules, tirets, apostrophes pour contourner les limites de \b avec l'Unicode
    const capWordRegex = /(?:^|[^a-zA-ZÀ-ÖØ-öø-ÿœŒæÆéèêëàâäôöûüç'-])([A-ZÀ-ÖØ-ß][a-zA-ZÀ-ÖØ-öø-ÿœŒæÆéèêëàâäôöûüç'-]*)/g;
    let match;
    while ((match = capWordRegex.exec(text)) !== null) {
      ...
      // Si le mot est déjà un alias généré dans la session, on ne le re-pseudonymise pas
      if (sessionState && sessionState.generatedAliases && sessionState.generatedAliases.some(alias => normalizeStr(alias) === normalizedWord)) {
        continue;
      }
      ...
      // OU s'il est immédiatement suivi par un autre mot capitalisé / connecteur (ce qui indique un nom propre composé)
      if (sentenceStart) {
    ...
    // On regroupe les mots capitalisés adjacents séparés uniquement par des espaces, tirets ou des connecteurs français
    ...
    while (i < wordCandidates.length - 1) {
      ...
      // Vérifier l'espace, le tiret séparateur ou un connecteur français (de, du, des, en, sur, sous, le, la, les, d')
      ...
      if (isAllowedGap) {
        ...
        // Déterminer le type final (si un composant est ORGANISATION, ou si la valeur contient un mot-clé ORG_KEYWORDS)
    ...
    for (let idx = finalCandidates.length - 1; idx >= 0; idx--) {
      ...
      if (mappedToken) {
        ...
        // Si c'est un NOM_PRENOM avec alias, on mappe également les parties individuelles (Prénom et Nom séparés)
